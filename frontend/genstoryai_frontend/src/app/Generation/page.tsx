...
function LabeledInput({
  ...
  return (
    <div className='flex items-center gap-1'>
      ...
      {fieldKey === 'gender' ? (
        readOnly ? (
          <span className='flex items-center flex-1 text-xs text-gray-800 min-h-[2rem]'>
            {GENDER_MAP[value as keyof typeof GENDER_MAP] || value}
          </span>
        ) : (
          <select
            value={value}
...
function RoleCardOptimized({
  ...
  return (
    <div
      className='bg-[#f6f8fa] rounded-2xl p-4 px-6 shadow-sm relative transition-all duration-300'
      style={{ minHeight: collapsed ? 56 : undefined }}
    >
      <div className='flex items-center'>
        ...
        <div className='ml-auto flex gap-2 items-center'>
          ...
          {!isEditing ? (
            <Edit
              className='w-4 h-4 cursor-pointer text-gray-400 hover:text-[#22b07d]'
              onClick={handleEdit}
            />
          ) : (
            <span
      ...
      {showAICard && isEditing && (
        <div
          className='absolute z-10 left-1/2 -translate-x-1/2 top-12 bg-white rounded-xl shadow-lg p-6 flex flex-col min-w-[340px] border border-gray-100'
          style={{ width: 360 }}
        >
          {/* 只保留提示词区 */}
      ...
      >
        <div className='grid grid-cols-2 gap-2 mb-2'>
          {(['name', 'gender', 'tag', 'age'] as (keyof typeof labelMap)[]).map(
            key => (
              <LabeledInput
                key={key}
                label={labelMap[key]}
...
export default function GenerationPage() {
  ...
  const [mainAllCollapsed, setMainAllCollapsed] = useState<boolean | null>(
    true
  ); // true:全部折叠，false:全部展开，null:不批量控制
  ...
  const [outlineLoading, setOutlineLoading] = useState<{ [key: number]: boolean }>({});
  ...
  return (
    <div className='min-h-screen bg-white flex flex-col'>
        ...
        <div className='flex items-center'>
          ...
          <div className='w-12 h-12 flex items-center justify-center mr-2'>
            <svg width='36' height='36' viewBox='0 0 36 36' fill='none'>
              ...
              />
              <path
                d='M12 22V16.5L18 12L24 16.5V22C24 22.5523 23.5523 23 23 23H13C12.4477 23 12 22.5523 12 22Z'
                stroke='#00AD88'
                strokeWidth='2'
      ...
      <div className='flex flex-1'>
        <div
          className='w-[400px] bg-white p-4 shadow-xl bg-[#f9fafb] rounded-2xl flex flex-col relative'
          style={{
          ...
          <div className='flex-1 overflow-y-auto'>
            ...
            <div>
              <div className='flex items-center justify-between mb-2'>
                <span
                  className='font-semibold text-base mr-4 cursor-pointer select-none'
                  onClick={() => {
          ...
          {showBook && (
            <div className='w-full max-w-2xl flex flex-col items-center'>
              <div className='flex items-center text-gray-600 text-base mb-4 w-full text-left'>
                ...
                {loadingOutline && (
                  <svg
                    className='animate-spin ml-2 w-5 h-5 text-blue-400'
                    viewBox='0 0 24 24'
                  >
                    <circle
              ...
              >
                {loadingOutline ? (
                  ...
                  <div
                    ...
                    style={{ maxHeight: 550, minHeight: 550 }}
                    ref={listRef}
                  >
                    {outlines.map((item, idx) => (
                      <div
                        key={idx}
                        ref={
                          idx === outlines.length - 1 ? lastCardRef : undefined
                        }
                      ...
                      >
                        ...
                        <div className='absolute top-3 right-4 flex gap-2 opacity-0 group-hover:opacity-100 transition'>
                          ...
                          <Trash2
                            ...
                            onClick={async () => {
                              if (outlineLoading[idx]) return;
                              setOutlineLoading(l => ({ ...l, [idx]: true }));
                              const newOutlines = outlines.filter((_, i) => i !== idx);
                              if (currentStory?.id) {
                                try {
                              ...
                              setOutlineLoading(l => ({ ...l, [idx]: false }));
                            }}
                          />
                        </div>
                        {editingIndex === idx ? (
              ...
              <button
                ...
                style={{
                  background:
                    'linear-gradient(90deg, #22b07d 0%, #3ad6a5 100%)',
                  zIndex: 10,
                }}
                onClick={() => {
      ...
      <Dialog open={showConfirmDialog} onOpenChange={setShowConfirmDialog}>
        <DialogContent>
          <DialogTitle>有未保存的章节</DialogTitle>
          <DialogDescription>是否保存当前未保存的故事章节？</DialogDescription>
          <div className='flex justify-end gap-2 mt-4'>
            ...
            <button
              ...
              onClick={async () => {
                if (editingIndex !== null) {
                  setOutlineLoading(l => ({ ...l, [editingIndex]: true }));
                  const newOutlines = outlines.slice();
                  newOutlines[editingIndex] = {
                    title: editTitle,
                    content: editContent,
                  };
                  const isModified =
                  ...
                  setOutlineLoading(l => ({ ...l, [editingIndex]: false }));
                }
              }}
            >
              确认
